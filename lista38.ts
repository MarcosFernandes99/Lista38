...
function BuscarTodos(id: number) {
 ...
 function BuscarPorId(id: number) {
    fetch (`https://apigenerator.dronahq.com/api/ZJa4lNKv/carros/${id}`)
   .then((response) => response.json())
   .then((data) => console.log(data)); 
}

 async function Cadastrar(data = {}) {
    const response = await fetch("https://apigenerator.dronahq.com/api/ZJa4lNKv/carros", {
...
async function AlterarValor(id: number, data = {}) {
    const response = await fetch(`https://apigenerator.dronahq.com/api/ZJa4lNKv/carros/${id}`, {
